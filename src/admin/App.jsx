...
// FIX: admin/App.jsx??ì¤‘ë³µ ?•ì˜?˜ì–´ ?ˆë˜ toMin, calcNightMinutesë¥?//      shared/utils?ì„œ import?˜ë„ë¡?ë³€ê²?import { normalizeDate, safeStr, calcWorkMinutes, calcNightMinutesSimple } from "../shared/utils";
...
function buildSettlement({ attendance = [], employees = [], month }) {
  ...
  const doneRows = attendance.filter((a) => {
    ...
    return (
      ...
      a.check_in &&
      a.check_out &&
      // FIX: toBool ë°©ì‹?¼ë¡œ ?µì¼ (GASê°€ true/false/boolean ?¼ìž¬ ë°˜í™˜)
  ...
  doneRows.forEach((a) => {
    ...
    // FIX: ì¤‘ë³µ ?•ì˜ ?œê±° ??shared/utils??calcNightMinutesSimple ?¬ìš©
...
export default function App() {
  ...
  const api = useApi({
    onError: useCallback((msg) => showToast(msg || "?¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤", "err"), [showToast]),
  });
