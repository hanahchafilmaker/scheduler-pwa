...
import "./AttTab.css"; // reuse same styles
...
// 파트별 표준 출근시간 (예정 스케줄이 없어도 이 기준으로 지각 계산)
...
function ApprovalModal({ row, onClose, onApprove, onReject }) {
  ...
  // (not using useEffect here to avoid extra renders; handled in parent)
...
// Helper options for part select
const PART_OPTIONS = [
  { value: "open", label: "오픈" },
...
export default function QCTab(props) {
  const {
    monthAttendance = [],
    approveAttendance,
    selectedMonth = "",
  ...
  return (
    <div className="att-tab">
      <div className="att-topbar">
        <div>
          <h2 className="att-title">품질 검사 (QC) - 출퇴근 승인</h2>
          <p className="att-subtitle">
            {selectedMonth || "-"} · 대기 중인 근태 기록을 검토하고 승인/거절 처리
          </p>
        </div>
      </div>
      ...
      <div className="att-summary-grid">
        ...
        <SummaryCard title="승인대기" value={summary.pending} sub="QC 대기 중" />
        <SummaryCard
          title="지급 근무시간"
