...
export function calcRowPayWithSeparation(row, hourlyWage) {
  ...
  if (isOutOfSchedule || !hasPlan) {
    extra = calcExtraLate(row.planned_end, row.check_out);
  } else {
    baseMin =
      diffSafe(row.planned_start, row.planned_end);
...
function diffSafe(a, b) {
  ...
  if (isNaN(ah + am + bh + bm)) return 0;
  return (bh * 60 + bm) - (ah * 60 + am);
}

export function calcPayrollLateDeductMinutes(row) { return calcLateDeduct(row.planned_start, row.check_in); }
export function calcPayrollEarlyLeaveDeductMinutes(row) { return calcEarlyLeaveDeduct(row.planned_end, row.check_out); }
