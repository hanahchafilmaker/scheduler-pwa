...
serve(async (req) => {
  ...
  try {
    ...
    if (!SMTP_USER || !SMTP_PASS) {
      return new Response(
        JSON.stringify({ success: false, error: "Missing SMTP credentials (SMTP_USER / SMTP_PASS)" }),
    ...
    // nodemailer transporter (한 번만 생성)
    ...
    for (const emp of employees) {
      ...
      if (!email || !email.includes("@")) {
        results.push({ employee_id, ok: false, reason: "invalid email" });
        continue;
      }
      if (!month) {
      ...
      try {
        ...
        results.push({ employee_id, email, ok: true });
      } catch (err) {
        results.push({ employee_id, email, ok: false, error: String(err) });
      }
